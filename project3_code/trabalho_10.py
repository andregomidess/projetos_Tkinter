...
class View():
    def __init__(self, master, controller):
        ...
        self.buttonClear = tk.Button(self.janela,text="Consulta código")      
        self.buttonClear.pack(side="left")
        self.buttonClear.bind("<Button>", controller.consultaCodigo) 
...
class Controller():       
    ...
    def enterHandler(self, event):
        ...
        codigoCli = int(self.view.inputText3.get())
    ...
    def consultaCodigo(self, event):
        codigo = askinteger("Consulta por código", "Digite o código")
        for clientes in  self.listaClientes:
            if codigo == clientes.getCodigo():
                email_nome = "Nome: " + clientes.getNome() + "\n" + "Email: " + clientes.getEmail()
                self.view.mostraJanela("Dados do cliente", email_nome)
            else:
