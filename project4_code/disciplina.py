...
class LimiteConsultaDisciplina():
    def __init__(self, str):
        messagebox.showinfo('Dados da disciplina', str)        
...
class CtrlDisciplina():       
    ...
    def consulta_disciplina(self):
        cod = askstring('Consulta por código', 'Digite o código')
        for disc in self.listaDisciplinas:
            if cod == disc.getCodigo():
                str = 'Código -- Nome\n'
                str += disc.getCodigo() + ' -- ' + disc.getNome() + '\n'
                self.limiteDisc = LimiteConsultaDisciplina(str)
                break
            else:
                messagebox.showinfo('Alerta', 'Código inexistente')
                break    
